...
def test(lib, handle, device, reduced_shape, data_shape, axes, keepdims, noop_with_empty_axes, tensor_dtype):
    print(
        f"Testing ReduceMean on {device} with reduced_shape:{reduced_shape} data_shape:{data_shape} axes:{axes} " \
    ...
    if isinstance(data_shape, tuple) and data_shape == (0,):
        # 如果是空集合，ans 应该为无穷小或最小值
        ans = torch.tensor(float("nan"), dtype=tensor_dtype).to(device)
    else: 
    ...
    if isinstance(data_shape, tuple) and data_shape == (0,):
        # 如果是空集合，reduced 为 nan
        assert reduced.ndim == 0 and torch.isnan(reduced).all()
    else:
        if tensor_dtype == torch.float16:
            assert torch.allclose(reduced, ans, atol=1e-6, rtol=1e-3)
        elif tensor_dtype == torch.float32:
            assert torch.allclose(reduced, ans, atol=1e-6, rtol=1e-5)
